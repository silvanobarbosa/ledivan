 ...
 */
/**
 * O modelo da mensagem de "Lembrar agendamento".
 *
 * Mesma ideia da de aniversário: o texto é da terapeuta e vale para todos os pacientes mensais.
 */
export async function salvarMensagemAgendamento(texto: string): Promise<{ ok: boolean; erro?: string }> {
  const session = await auth();
  if (!session?.user?.id) return { ok: false, erro: "Não autorizado" };

  const limpo = (texto ?? "").trim().slice(0, 1000);
  try {
    await db.update(users).set({ agendamentoMessage: limpo || null }).where(eq(users.id, session.user.id));
