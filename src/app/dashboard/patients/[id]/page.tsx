/* eslint-disable react-hooks/purity, react-hooks/immutability --
   ...
   requisição e nunca re-renderiza no navegador. As regras acima vêm do React Compiler e
   ...
   achados de verdade — eles passam a valer. */
import { db } from "@/db";
...
export default async function PatientPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const lastRealizada = realizadas.map((s) => new Date(horaDeParede(s.date)).getTime()).sort((a, b) => b - a)[0];
  const sessionStats = {
    reservadas: sessionsList.filter((s) => s.pendingConfirmation && new Date(horaDeParede(s.date)).getTime() >= nowMs).length,
    agendadasFuturas: sessionsList.filter((s) => s.status === "agendada" && new Date(horaDeParede(s.date)).getTime() >= nowMs).length,
  ...
  const recSessions = sessionsList.filter((s) => s.recurring && new Date(horaDeParede(s.date)).getTime() >= nowMs).sort((a, b) => new Date(horaDeParede(a.date)).getTime() - new Date(horaDeParede(b.date)).getTime());
