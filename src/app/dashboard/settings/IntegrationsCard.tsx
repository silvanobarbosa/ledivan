...
import { Calendar, Mail, MessageCircle, Check, Plug } from "lucide-react";
import { setIntegration } from "./actions";
import type { Integrations } from "@/lib/preferences";
...
export function IntegrationsCard({ initial }: { initial: Integrations }) {
  ...
  const toggle = (key: "googleCalendar" | "gmail" | "whatsapp") => {
    ...
    setState(next);
    startTransition(() => setIntegration({ [key]: next[key] }));
  };

  const saveNumber = (whatsappNumber: string) => {
        ...
        {items.map((it) => {
          ...
          return (
            <div key={it.key} className="space-y-3">
              <div className="flex items-center gap-4 p-4 rounded-2xl bg-surface/50 border border-border">
                ...
                <button
                  ...
                  className={`shrink-0 px-4 py-2 rounded-xl text-sm font-bold transition ${
                    connected ? "bg-[#ecfdf5] text-[#047857]" : "bg-primary text-white hover:scale-[1.02]"
                  }`}
                >
              ...
              {/* Campo número do WhatsApp quando conectado */}
