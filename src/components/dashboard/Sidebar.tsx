...
export function NavList({ onNavigate, className }: { onNavigate?: () => void; className?: string }) {
  ...
  return (
    <nav className={cn("flex-1 min-h-0 px-4 space-y-1 overflow-y-auto overscroll-contain", className)}>
      {NAV_GROUPS.map((group, i) => (
        /*
         * Os tres titulos se parecem. O grupo Financeiro tinha caixa azul propria e TRES `pt-`
         * brigando na mesma string — era o "FINANCEIRO solto" que a dona fotografou no celular
         * (18/09). Um menu nao precisa de um grupo em destaque; precisa de tres iguais.
         */
        <div key={group.title}>
          <p className={cn("px-6 pb-1 text-[10px] font-bold uppercase tracking-widest text-foreground/40", i === 0 ? "pt-2" : "pt-5")}>
            {group.title}
          </p>
